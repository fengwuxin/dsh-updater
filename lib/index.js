...
// 模块解析锚定到运行中的 dsh CLI 安装树：process.argv[1] 指向 dsh 可执行文件（bin 下的软链），
// 其上级两级是 node 版本根，其下的 lib/node_modules/@deepseek-ai/dsh 是 CLI 包本体。
const CLI_PKG_JSON = join(dirname(dirname(process.argv[1])), "lib", "node_modules", "@deepseek-ai", "dsh", "package.json");
const cliRequire = createRequire(CLI_PKG_JSON);
const z = cliRequire("@deepseek-ai/schemastery");

const name = "dsh-updater";
const inject = ["timer", "webServer"];

/** 插件配置（profile 补丁可通过 config 覆盖）。 */
const CHANNELS = ["both", "next", "latest"];
const Config = z.object({
	/** 检测渠道：both=latest 与 next 里取较新者；next=只跟踪预发布渠道；latest=只跟踪稳定渠道。 */
	channel: z.string().default("both"),
	/** 每天检查的本地时刻（0-23 点）。 */
	checkHour: z.number().min(0).max(23).default(10),
	/** npm registry 基础地址。 */
	registry: z.string().default("https://registry.npmjs.org"),
	/** 升级完成后是否自动重启服务（重启会短暂断开当前网页连接）。 */
	autoRestart: z.boolean().default(true),
	/** 升级完成后到重启的延迟（毫秒），用于让 HTTP 响应先落盘。 */
	restartDelayMs: z.number().min(0).default(3000)
});

/** 读取已安装的 @deepseek-ai/dsh 版本（从运行中的 CLI 安装读取）。 */
function readInstalledVersion() {
	try {
		return JSON.parse(readFileSync(CLI_PKG_JSON, "utf8")).version ?? null;
	} catch {
		return null;
	}
}
